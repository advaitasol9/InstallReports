    ...
    managerQuestions.forEach((question) => {
      if (question.required) {
        if (question.type === "signature" && this.props.signature.length == 0) {
          ...
          this.props.photos.filter((photo) => photo.order == question.order)
            .length == 0
        ) {
          isAllRequiredQuestionsAnswered = false;
        } else if (
  ...
  saveManagerQuestionAnswers = async () => {
    if (
      ...
      await apiPatchAnswers(
        ...
        {
          manager_questions_answers: JSON.stringify(
      ...
      return (
        ...
        >
          ...
          <NavigationEvents
            onWillFocus={() => {
              if (this.props.connectionStatus) {
                apiGetJson(
                  `activities/${this.props.activityId}?with=["items"]`,
                  this.props.token
                ).then(async (response) => {
                  const installerAnswers = JSON.parse(
                    response.data.installer_questions_answers
                  );
                  this.props.setActivityData({
                    ...response.data,
                    manager_questions_answers: JSON.parse(
                      response.data.manager_questions_answers
                    ),
                  });
                  this.props.setIsloading(false);
                  if (
                    installerAnswers.length == 0 ||
                    installerAnswers.filter((answer) => answer.answers != "")
                      .length == 0
                  ) {
                ...
                this.props.setActivityData(
                  this.props.orderList.filter(
                    (order) => order.id === this.props.activityId
                  )[0]
                );
                this.props.setIsloading(false);
              }
                ...
                <Button
                  ...
                  onPress={async () => {
                              ...
                              .then(() => {
                                RNFetchBlob.fs
                                  ...
                                  .then((stats) => {
                                    ...
                                    formData.append(
                                      "s3_location",
                                      res.data.file_name.replace("uploads/", "")
                                    );
                                    ...
                                    apiPostImage(
                                      ...
                                      .then((fileRes) => {
                                        this.props.activityData.manager_questions_answers.forEach(
                                          (question, index) => {
                                            if (question.order === item.order) {
                                              if (question.type == "photo") {
                                                if (
                                                  this.props.activityData
                                                    .manager_questions_answers[
                                                    index
                                                  ].answers == undefined ||
                                                  this.props.activityData
                                                    .manager_questions_answers[
                                                ...
                                                if (
                                                  ...
                                                  this.props.activityData.manager_questions_answers[
                                                    index
                                                  ].photo = [fileRes.data.id];
                                                } else {
                                                  this.props.activityData.manager_questions_answers[
                                                    index
                                                  ].photo.push(fileRes.data.id);
                                                }
                                              }
                                            }
                                          }
                    ...
                    if (this.props.signature.length == 1) {
                      let signatureQuestionIndex = this.props.activityData.manager_questions_answers.findIndex(
                        (question) => question.type == "signature"
                      );
                      let signatureQuestion = this.props.activityData
                        .manager_questions_answers[signatureQuestionIndex];
                      if (signatureQuestionIndex != -1) {
                        apiGet("aws-s3-presigned-urls", this.props.token).then(
                          (res) => {
                            RNFetchBlob.fetch(
                              "PUT",
                              res.data.url,
                              ...
                              RNFetchBlob.wrap(
                                this.props.signature[0].replace("file://", "")
                              )
                            )
                              .then(() => {
                                RNFetchBlob.fs
                                  .stat(
                                    this.props.signature[0].replace(
                                      "file://",
                                      ""
                                    )
                                  )
                  ...
                  caption="Submit"
                  bgColor={
                    this.state.isSubmitBtnDisabled ? "#b1cec1" : colors.green
                  }
          ...
          {this.props.isIncompleteOpen && (
            <IncompleteModal
              close={() => this.props.setIsIncompleteOpen(false)}
            />
          )}
        </KeyboardAvoidingView>
      );
